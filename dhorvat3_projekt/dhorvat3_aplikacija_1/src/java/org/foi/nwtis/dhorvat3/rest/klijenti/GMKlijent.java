...
public class GMKlijent {
    ...
    public Lokacija getGeoLocation(String adresa){
        try{
            ...
            JsonObject obj = jo.getJsonArray("results").getJsonObject(0).getJsonObject("geometry").getJsonObject("location");
    ...
    public String getGeoAddress(float lattitude, float longitude){
        WebTarget webResource = client.target(GMRESTHelper.getGM_BASE_URI()).path("maps/api/geocode/json");
        webResource = webResource.queryParam("latlng", lattitude + "," + longitude);
        webResource = webResource.queryParam("sensor", "false");
